...
- Evidence: `artifacts/desktop-control/combined-lote-smoke/20260624-six-fronts-default-altspace-uia-observer/report.json`. The initial post-change full E2E attempts were blocked before dictation by Windows foreground-lock rejecting CUA `bring_to_front`; the harness was hardened with an `AttachThreadInput` focus recovery and `-DictationKey AltSpace`, then full default Alt+Space E2E passed at `artifacts/desktop-control/dictation-e2e/20260624-e2e-altspace-default/report.json`.
