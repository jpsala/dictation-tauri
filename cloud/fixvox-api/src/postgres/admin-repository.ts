...
function profileCapabilities(profileId: string): string[] {
  if (profileId === "alpha-basic") return ["dictation", "postprocess", "managed_stt", "managed_llm", "vocabulary"];
  if (profileId === "alpha-full") return ["translate", "dictation", "postprocess", "selection_transform", "assistant_actions", "custom_prompts", "advanced_settings", "managed_stt", "managed_llm", "vocabulary"];
  if (profileId === "power-admin") return ["translate", "dictation", "postprocess", "selection_transform", "assistant_actions", "custom_prompts", "advanced_settings", "debug_tools", "managed_stt", "managed_llm", "admin_settings", "vocabulary"];
  if (profileId === "pro") return ["translate", "dictation", "postprocess", "selection_transform", "assistant_actions", "custom_prompts", "advanced_settings", "managed_stt", "managed_llm", "vocabulary"];
  return [];
}
...
export class PostgresAdminRepository {
  ...
  private async workerCatalog(): Promise<{ engineOptions: Record<string, unknown>[]; promptOptions: Record<string, unknown>[]; variantOptions: Record<string, unknown>[]; groupOptions: Record<string, unknown>[]; selectionPresets: Record<string, unknown> | null }> {
    ...
    const engineOptions = new Map<string, Record<string, unknown>>(BUILTIN_ENGINES.map((engine) => [engine.id, { id: engine.id, label: engine.label, kind: engine.kind, tier: engine.tier, provider: engine.provider, model: engine.model, notes: engine.notes, promptKey: engine.promptKey, promptSummary: engine.promptSummary, supportedEfforts: engine.supportedEfforts, defaultEffortId: engine.defaultEffortId, lifecycleStatus: "published", availability: "available", source: engine.source }]));
    for (const row of engines) {
      ...
      engineOptions.set(row.engine_id, {
        ...
        supportedEfforts: Array.isArray(options.supportedEfforts) ? options.supportedEfforts : [],
        defaultEffortId: typeof options.defaultEffortId === "string" ? options.defaultEffortId : null,
  ...
  async assignAccountPolicy(input: { accountHandle: string; policyId: string; actorRefHash: string; expectedCurrentProfileId?: string }) {
    const accountHandle = input.accountHandle.trim();
    const policyId = input.policyId.trim();
    const expectedCurrentProfileId = input.expectedCurrentProfileId?.trim();
    if (
      !/^[a-z0-9][a-z0-9_-]{0,63}$/.test(accountHandle)
      || !/^[a-z0-9][a-z0-9-]{0,63}$/.test(policyId)
      || (expectedCurrentProfileId !== undefined && !/^[a-z0-9][a-z0-9-]{0,63}$/.test(expectedCurrentProfileId))
    ...
    return this.sql.begin(async (tx) => {
      ...
      const alreadyAssigned = currentProfileId === policyId;
      if (alreadyAssigned) {
      ...
      if (expectedCurrentProfileId !== undefined && currentProfileId !== expectedCurrentProfileId) throw new Error("account_profile_conflict");
      ...
      `, [input.actorRefHash, accountHandle, { schemaVersion: 1, profileId: policyId, previousProfileId: currentProfileId, devicesUpdated: devices.length }]);
      return {
        ok: true as const,
        devicesUpdated: devices.length,
  ...
  async catalog() { const [engines, prompts, groups] = await Promise.all([this.sql.unsafe<{ engine_id: string; kind: string; provider: string; model: string; provider_label: string; model_label: string; tier: string; supported_efforts: unknown; default_effort_id: string | null; enabled: boolean; lifecycle_status: string; availability: string; published_revision: string | null; catalog_revision: string | number; reviewed_at: string | null; reviewed_by: string | null; source: string }>(`SELECT engine_id, kind, provider, model, provider_label, model_label, tier, supported_efforts, default_effort_id, enabled, lifecycle_status, availability, published_revision, catalog_revision, reviewed_at::text, reviewed_by, source FROM engines ORDER BY engine_id`), this.sql.unsafe<{ prompt_id: string; kind: string; version: number; enabled: boolean }>(`SELECT prompt_id, kind, version, enabled FROM prompts WHERE enabled ORDER BY prompt_id`), this.sql.unsafe<{ group_id: string; label: string; description: string | null; runtime_profile_id: string | null; source: string }>(`SELECT group_id, label, description, runtime_profile_id, source FROM groups ORDER BY group_id`)]); return { engineOptions: engines.map((r) => ({ id: r.engine_id, kind: r.kind, provider: r.provider, model: r.model, providerLabel: r.provider_label, modelLabel: r.model_label, tier: r.tier, supportedEfforts: jsonValue(r.supported_efforts), defaultEffortId: r.default_effort_id, enabled: r.enabled, lifecycleStatus: r.lifecycle_status, availability: r.availability, publishedRevision: r.published_revision, revision: Number(r.catalog_revision), reviewedAt: r.reviewed_at, reviewedBy: r.reviewed_by, source: r.source })), promptOptions: prompts.map((r) => ({ id: r.prompt_id, kind: r.kind, version: `v${r.version}`, source: "postgres" })), groupOptions: groups.map((r) => ({ id: r.group_id, label: r.label, description: r.description, policyId: r.runtime_profile_id, source: r.source })) }; }
