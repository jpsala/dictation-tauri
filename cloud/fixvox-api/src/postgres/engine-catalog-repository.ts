...
export class PostgresEngineCatalogRepository implements EngineCatalogStore {
  ...
  private async mutate(input: { engineId: string; actorRef: string; expectedRevision?: number; occurredAt: string }, action: "publish" | "retire" | "review"): Promise<{ entry: EngineCatalogEntry; audit: EngineCatalogAudit; idempotentReplay: boolean }> {
    return this.sql.begin(async (tx) => {
      ...
      await tx.unsafe("INSERT INTO engine_catalog_audits (actor_ref_hash, action, engine_id, previous_status, resulting_status, previous_revision, resulting_revision, safe_metadata, occurred_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9::timestamptz)", [input.actorRef, action, input.engineId, currentEntry.lifecycleStatus, targetStatus, currentEntry.revision, nextRevision, JSON.stringify({ schemaVersion: 1 }), input.occurredAt]);
