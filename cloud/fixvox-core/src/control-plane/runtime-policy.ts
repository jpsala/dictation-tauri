import { asJsonRecord, cloneJsonValue, type JsonRecord } from "./policy-values.ts";
...
function readSelectionPresetDefaults(record: JsonRecord | null): RegisterUserSettingsDefaults["selectionPresets"] | undefined {
  ...
  return {
    ...
    items: items.map((item) => asJsonRecord(item)).filter((item): item is JsonRecord => Boolean(item)).map((item) => ({
...
export function buildDefaultRuntimePolicy(): JsonRecord {
  return cloneJsonValue(RECOMMENDED_ALPHA_RUNTIME_POLICY);
}
...
export function validateRuntimePolicy(policy: JsonRecord): void {
...
export function buildRegisterDefaultsFromRuntimePolicy(policy: JsonRecord, cohorts: string[] = []): JsonRecord {
  ...
  const { voiceRouting: _ignoredVoiceRouting, ...policyWithoutVoiceRouting } = cloneJsonValue(policy);
  const userSettingsDefaults = readUserSettingsDefaults(policy);

  const normalizedLlmTargets = {
    ...cloneJsonValue(llmTargets ?? {}),
