...
- Durante trabajo de producto, mantener la app real Tauri activa y actualizada siempre que sea razonable: verificar `npm run tauri:dev`/`dictation-tauri.exe` antes de iterar, reiniciar tras cambios que HMR no cubra y no sustituirla por Vite/browser cuando haya tray, hotkeys, ventanas nativas o shell Tauri. Cuando Pi arranque la app para uso en vivo de JP, preferir `npm run tauri:dev:hidden -- -StopExisting` para no abrir terminales ni robar foco.
