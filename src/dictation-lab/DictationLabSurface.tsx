    ...
    <main className="lab-shell">
      ...
      <div className="lab-frame">
        ...
        <section className="lab-main" aria-label={`${labWorkspaces.find((item) => item.id === workspace)?.label ?? "Laboratory"} workspace`}>
          ...
          {workspace === "experiments" ? <ExperimentWorkspace artifacts={artifacts} definition={experiment} estimate={estimate} estimateLoading={estimateLoading} estimateError={estimateError} job={job} orchestrationAvailable={jobsAvailable} providerAuthorizationAvailable={load.catalog?.providerAuthorization.status === "available"} availableRecipeIds={{ stt: (load.catalog?.sttRecipes ?? []).filter((entry) => entry.executionModes.includes(experiment.mode)).map((entry) => entry.id), postprocess: (load.catalog?.postprocessRecipes ?? []).filter((entry) => entry.availability.status === "available" && entry.executionModes.includes(experiment.mode)).map((entry) => entry.id), materialization: (load.catalog?.materializations ?? []).filter((entry) => entry.availability.status === "available" && entry.executionModes.includes(experiment.mode)).map((entry) => entry.id), prosody: (load.catalog?.prosodyModes ?? []).filter((entry) => entry.availability.status === "available" && entry.executionModes.includes(experiment.mode)).map((entry) => entry.id as LabExperimentDefinition["prosodyModes"][number]), vocabulary: (load.catalog?.vocabularyModes ?? []).filter((entry) => entry.id === "off" || (load.vocabularySnapshots.length > 0 && entry.snapshotPrerequisite.required)).filter((entry) => entry.executionModes.includes(experiment.mode)).map((entry) => entry.id as LabExperimentDefinition["vocabularyModes"][number]) }} onChange={changeExperiment} onEstimate={() => void estimateExperiment()} onStart={() => void startExperiment()} onCancel={() => void cancelExperiment()} onReload={() => void reloadJob()} onRefreshArtifacts={() => void refreshArtifacts()} /> : null}
