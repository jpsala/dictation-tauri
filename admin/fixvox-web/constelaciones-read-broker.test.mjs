...
test('Constelaciones broker returns only bounded future appointment projection', async () => {
  ...
  db.prepare('INSERT INTO appointments VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run('appt-secret', 'slot-secret', '2026-07-20T10:00:00.000Z', '2026-07-20T11:00:00.000Z', 'confirmed', 'Person Secret', '+54000', 'private note')
  db.close()
  ...
  try {
    ...
    assert.deepEqual(payload.rows, [{ startsAt: '2026-07-20T10:00:00.000Z', endsAt: '2026-07-20T11:00:00.000Z', status: 'confirmed', location: 'Mar del Plata' }])
