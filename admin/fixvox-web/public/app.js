...
function renderAccountWorkbench() {
  return `<div class="admin-workbench"><div class="workbench-head"><div><span class="eyebrow">Mi cuenta</span><h2>${esc(state.env?.user?.name || 'Admin')}</h2><p>Sesión web admin autenticada server-side. Los secrets y API keys no se exponen al browser.</p></div><a class="button" href="/logout">Salir</a></div><div class="entity-grid compact"><article class="entity-card"><strong>${esc(state.env?.user?.email || 'email no disponible')}</strong><div class="entity-meta"><span>${esc(state.env?.user?.provider || 'admin')}</span><span>${esc(state.env?.environment || 'env')}</span></div></article><article class="entity-card"><strong>Seguridad</strong><div class="entity-meta"><span>Mutaciones production requieren confirmación PROD.</span></div></article></div></div>`
}
function accountEmail(account) { return account.userEmail || account.userEmailRedacted || null }
function accountDisplayName(account) { return account.displayName || accountEmail(account) || account.userRedacted || account.emailRedacted || account.accountHandle || 'usuario redacted' }
function accountSecondaryLabel(account) {
  return [accountEmail(account), account.provider, account.accountHandle || account.accountIdRedacted || 'account redacted'].filter(Boolean).join(' · ')
}
...
function renderAccountOverview(account) {
  ...
  const variants = accountVariants(account)
  const budget = account.accountBudget
  const budgetLabel = budget
    ? [`USD ${budget.dailyUsd ?? 'sin límite'} diario`, `USD ${budget.monthlyUsd ?? 'sin límite'} mensual`, budget.mode].join(' · ')
    : 'Hereda el budget del profile'
  const facts = [
    ['Email', accountEmail(account) || 'No disponible'],
    ['Proveedor', account.provider || 'No disponible'],
    ['Account handle', account.accountHandle || account.accountIdRedacted || 'No disponible'],
    ['Profile efectivo', effectivePolicyLabel(account)],
    ['Fuente del profile', policySourceLabel(account.effectivePolicySource || (account.policyId ? 'account' : 'base'))],
    ['Devices', account.deviceCount ?? (Array.isArray(account.devices) ? account.devices.length : 0)],
    ['Última actividad', formatDateTime(account.lastSeenAt)],
    ['Budget', budgetLabel],
    ['Groups', groups.length ? groups.map(groupLabel).join(', ') : 'Ninguno'],
    ['Overrides', variants.length ? variants.map(segmentLabel).join(', ') : 'Ninguno'],
  ]
  return `<section class="account-overview" aria-label="Datos completos de la cuenta"><h4>Datos de la cuenta</h4><dl class="account-facts">${facts.map(([label, value]) => `<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join('')}</dl></section>`
}
function renderAccountDetail(account, policyOptions) {
  const devices = Array.isArray(account.devices) ? account.devices : []
  return `<section class="account-detail"><div class="entity-card-head"><div><span class="eyebrow">Cuenta seleccionada</span><h3>${esc(accountDisplayName(account))} ${currentAccountBadge(account)}</h3><small>${esc(accountSecondaryLabel(account))}</small></div>${renderEffectivePolicyBadge(account)}</div>${renderAccountOverview(account)}<div class="policy-control" role="group" aria-label="Policy actual">${policyOptions.map((policy) => { const id = typeof policy === 'string' ? policy : policy.policyId || policy.id || ''; const label = typeof policy === 'string' ? policy : policy.policyLabel || policy.label || id; const active = id === account.policyId; return `<button class="policy-option ${active ? 'active' : ''}" data-preview-account-policy="${esc(account.accountHandle)}" data-policy="${esc(id)}" ${active ? 'disabled' : ''}>${esc(label)}</button>` }).join('') || '<span class="muted">Sin opciones de policy.</span>'}</div>${renderAccountPolicyPreview(account, policyOptions)}${renderAccountBudget(account)}${renderAccountGroups(account)}${renderEffectiveSettings(account)}<div class="linked-devices"><strong>Devices vinculados</strong>${devices.length ? `<div class="linked-device-list">${devices.map((device) => `<article class="linked-device"><div><strong>${esc(device.deviceIdRedacted || 'device redacted')}</strong><small>${esc(formatDateTime(device.lastSeenAt))}</small></div><span class="chip ${device.status === 'active' ? 'ok' : ''}">${esc(device.status || 'unknown')}</span><span class="policy-badge">${esc(device.policyLabel || device.policyId || 'none')}</span><button class="button tiny" disabled>Revocar</button></article>`).join('')}</div>` : '<p class="muted">Este endpoint todavia no devolvio devices vinculados para esta cuenta.</p>'}</div></section>`
}
