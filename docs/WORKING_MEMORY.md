...
- **Próximo batch/corte:** rollout completo autorizado, detenido antes de deploy. Commit/push inicial `450cd3d` y build/upgrade OK; smoke detectó contrato legacy y carrera packaged al abrir Settings. Corrección host lista: smoke exige dock oculto + Settings + cero provider/login/clipboard; `show_account_setup_window` espera hasta 2,5 s sólo por la ventana configurada, con timeout redacted y sin fallback durante startup; tests Rust/source verdes. Próximo: commit/push correctivo → rebuild/upgrade/smoke verde; recién después Worker, Admin y prerelease. No incluye login/link ni provider/dictado.
