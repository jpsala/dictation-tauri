...
- **Próximo batch/corte:** rollout completo autorizado, detenido antes de deploy. Commit/push inicial `450cd3d` y build/upgrade local OK; smoke detectó contrato legacy (esperaba dock visible) y una carrera packaged al abrir Settings. Corrección lista: smoke exige dock oculto + Settings + cero provider/login/clipboard; test nuevo; account gate reintenta una sola vez tras 300 ms. Próximo: checks → commit/push correctivo → rebuild/upgrade/smoke verde; recién después Worker, Admin y prerelease. No incluye login/link ni provider/dictado.
