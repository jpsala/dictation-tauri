...
- **Próximo batch/corte:** Batch 2 autorizado. Split provider/workspace implementado local: `--no-builtin-tools`, custom read/write/edit/bash por Unix socket y broker con roots/secrets/env/output/timeout fail-closed; 26 tests + RPC offline PASS. Próximo: commit/push, test Linux provider-free, crear usuarios/grupos/directorios y unidades con backup; luego provider OAuth aislado, Constelaciones broker y smokes. Stop si Pi ve OAuth+workspace simultáneamente. Browser local queda para relay posterior.
