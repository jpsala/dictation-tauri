...
- **Próximo batch/corte:** Batch 5 scope Dictation push + Admin deploy. Foundations `8bb6668`/`622e9ac`; provisioning local listo: exact Admin Unix helper+operations, units disabled, root configs, repo-specific key registration y dry-run gated. Linux fake Git/push/deploy/rollback + HTTP 9/9 PASS; no credenciales creadas. Falta commit/push y gate exacto para `pi-release-provision.ps1 -ConfirmProduction -RegisterDeployKey` (sin enable). Enable tools y primer commit/push/deploy requieren otro gate. No conceder credenciales al agent/workspace.
