...
- **Próximo batch/corte:** sigue siendo sandbox, no paridad total. Pendientes separados: rollout/sync durable de `/opt`+mirrors, grep/find/ls autónomos, git/deploy credentials específicas y relay Chrome/desktop. No conceder Docker/sudo/SSH ni homes/stores.
