...
23. Dev dock debe quedar siempre corriendo durante trabajo de usabilidad: usar `powershell -ExecutionPolicy Bypass -File scripts/dev-dock.ps1`, y `-Restart` tras cambios Rust/Tauri/config/hotkey/window; renderer/CSS/TS hot-refresh via Vite. Smoke computer-use 2026-06-23 dejo `dictation-tauri` vivo y probo `Alt+Space` + fallback hasta fresh WAV, pero no insercion (target vacio, clipboard sentinel restaurado), por lo que no hay `paste_observed`.
24. Proximo recomendado: resolver por que el smoke automatizado no llega a insercion/transcript en target controlado, luego native observer Windows gated (UI Automation o fixture controlado) o target heuristics antes de llamar al paste “verificado”; alternativa si JP prioriza app usable: tray/background como siguiente spec/checkpoint.
25. Checks recientes: `npm run test:pipeline` OK (50 files / 241 tests), `npm run build` OK, `npm run visual:check` OK (8 tests), `cd src-tauri && cargo check` OK. Previos: `bun scripts/context-index.ts && bun scripts/agent-context-audit.ts` OK con 4 warnings conocidos. `cd src-tauri && cargo test desktop_control` no fue usado como gate: falla en este entorno con `STATUS_ENTRYPOINT_NOT_FOUND`.
