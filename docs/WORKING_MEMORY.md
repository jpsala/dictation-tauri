...
20. Closeout `012` T024-T028 quedo hecho para el alcance safe shortcut/dev dock/saved-target `paste_sent`; tray/background, selection/replace y observer/verificacion real quedan future/gated.
21. JP recalco que la direccion viene de `C:\dev\fixvox` y hay que hacer que funcione como Fixvox: `Alt+Space` quedo code-enabled como dictation key primaria Tauri via `tauri-plugin-global-shortcut`, con `Ctrl+Shift+F9` aun registrado como fallback; renderer acepta ambos en el resolver hold/tap. Manual Windows smoke de Alt+Space queda pendiente antes de declararlo probado.
22. `013-verified-paste-observer` arranco por la ruta recomendada de observer/verification: se agrego seam provider-free (`src/delivery/observation.ts`) que solo promueve a `paste_observed` con observer high-confidence explicito, mantiene `paste_sent`/`uncertain` en otros casos y permite inyectar observer opcional en `createTauriSavedTargetDeliveryGateway` sin cambiar default.
23. Dev desktop canonico: usar `npm run dev:desktop` (alias de `tauri dev`, alineado con Tauri/Fixvox) para mantener Vite + `Dictation Dock`; usar `npm run dev:desktop:restart` solo como recovery tras cambios Rust/Tauri/config/hotkey/window o procesos stale; `npm run dev:desktop:status` para verificar. El helper interno `scripts/dev-dock.ps1` mata stale `dictation-tauri` + duenos del puerto 1420/Vite antes de relanzar.
24. `Needs attention` del smoke se diagnostico: faltaba `.env` local y managed Fixvox requeria install/device; luego JP pidio copiar `.env` desde `ssh pc:C:\dev\dictation-tauri\.env` a `.env` local gitignored sin imprimir secretos. Rust cae a direct Groq BYOK si managed setup falla pero Groq esta configurado.
25. Fix de insercion Alt+Space/fallback: el payload nativo de hotkey ahora trae el target foreground capturado sincronicamente en Rust antes de que el renderer/dock cambie foco, y desktop delivery manda Escape antes de Ctrl+V para cerrar el menu de sistema que Alt+Space puede dejar abierto. Computer-use smoke 2026-06-23 paso con target controlado: `Alt+Space` creo `capture-native-1782248488074.wav` / `capture-native-1782248980021.wav` e inserto contenido redacted; `Ctrl+Shift+F9` creo `capture-native-1782248836922.wav` / `capture-native-1782249026565.wav` e inserto contenido redacted; clipboard sentinel restaurado. Semantica sigue `paste_sent`, no `paste_observed`.
26. `013` Checkpoint B code quedo implementado: `observe_desktop_paste` en Rust observa superficies de texto Win32 del target guardado, no devuelve contenido crudo, y el renderer solo lo inyecta con `VITE_ENABLE_NATIVE_PASTE_OBSERVER=1`; default sigue `paste_sent`.
...
31. `014` arranco como spec de paridad Fixvox para tray/settings/hotkeys/quick-pick/postprocess/focused-input. Checkpoint A quedo implementado: Tauri `tray-icon`, `src-tauri/src/tray.rs`, menu IDs estables `show_dock`/`hide_dock`/`settings`/`quit`, left-click toggle, `skipTaskbar: true` para que el dock no aparezca en taskbar, y `npm run dev:desktop:refresh` para re-mostrar/topmost el dock ya instanciado sin matar la app.
32. Analisis Fixvox 2026-06-23 actualizado en `docs/topics/fixvox-dock-and-hotkeys-reference.md`: falta no-activate real, drag/persistencia, monitor-follow idle, context menu, companion, settings, quick-pick y postprocess/routing. Para drag, preferir comando Rust/Tauri `start_dragging()` y persistir posicion; fallback Win32 manual si Tauri falla con ventana transparente/no-taskbar.
33. Proximo recomendado: `014` Checkpoint B dock polish (no-activate + drag/persistencia) antes de settings/hotkey schema, luego quick-pick y postprocess seam.
34. Checks recientes: `npm run test:pipeline` OK (53 files / 250 tests), `npm run build` OK, `cd src-tauri && cargo check` OK, dev desktop restart OK, context index/audit OK con warnings conocidos. Previos: `npm run visual:check` OK (8 tests). `cd src-tauri && cargo test desktop_control` no fue usado como gate: falla en este entorno con `STATUS_ENTRYPOINT_NOT_FOUND`.
