...
- **Próximo batch/corte:** Release tools fail-closed `OFF`; key/services siguen provisionados. Wording fix + regression local PASS (security/Admin 38/38): tool call dispara approval card, no pedirla sólo en prosa. Segundo smoke autorizado pre-sync `20260718-210101`: stream automation duplicó request (una activa/otra 409), así se abortó antes de commit; único edit restaurado, mirror==remote `34ad7bd` clean, journal 0, no push/deploy. Próximo: commit fix/docs y gate nuevo para rollout+sync/re-enable read-only; repetir sólo con UI requests separadas observables.
