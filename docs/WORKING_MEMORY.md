...
26. `013` Checkpoint B code quedo implementado sin smoke manual: `observe_desktop_paste` en Rust observa superficies de texto Win32 del target guardado, no devuelve contenido crudo, y el renderer solo lo inyecta con `VITE_ENABLE_NATIVE_PASTE_OBSERVER=1`; default sigue `paste_sent`.
27. Proximo recomendado: smoke controlado del observer gated contra Notepad/target fixture para probar promocion real a `paste_observed`; alternativa si JP prioriza app usable: tray/background como siguiente spec/checkpoint.
28. Checks recientes: focused observer tests OK (`delivery-observation`, `native-paste-observer`, `desktop-delivery-rust`), `npm run build` OK, `cd src-tauri && cargo check` OK. Previos: `npm run test:pipeline` OK (51 files / 242 tests), `npm run visual:check` OK (8 tests), `bun scripts/context-index.ts && bun scripts/agent-context-audit.ts` OK con warnings conocidos. `cd src-tauri && cargo test desktop_control` no fue usado como gate: falla en este entorno con `STATUS_ENTRYPOINT_NOT_FOUND`.
