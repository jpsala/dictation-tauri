...
- **Estado:** sandbox remoto construido y smokes técnicos PASS, pero feature producción volvió a `0` al detectar que Pi routes sólo exigían sesión global y compartían RPC. Fix local agrega owner-Google-only, token/viewer/editor 403, prompt serial global, confirm session-bound/operation-hash/TTL/one-time y OAuth reciente; tests verdes. Brokers aislados y Constelaciones read-only siguen activos sin exposición web.
- **Próximo batch/corte:** commit/deploy del cierre RBAC con feature aún off; reactivar sólo con smoke owner Google real. No iniciar OAuth Browser sin gate explícito. Luego: sync controlado de mirrors, grep/find/ls, git/deploy credentials específicas y relay Chrome/desktop. No conceder Docker/sudo/SSH ni homes/stores.
