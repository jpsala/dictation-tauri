...
- **Próximo batch/corte:** Batch 5 Git/deploy foundation listo local y feature default/prod `0`: broker/runner separados, typed ops/recipes, exact branch/remote/hash/FF, allowed commit paths, nonce+TTL+typed phrase, recent owner UI binding, bounded journal y rollback; bash normal bloquea git mutations/systemctl/Docker/Wrangler/SSH/SCP/deploy scripts. Tests fake + Git real + deploy rollback verdes. Falta commit/push y gate separado para crear `fixvox-release`, deploy key repo-specific, socket/service/config. Primer commit/push/deploy smoke requiere otro gate. No conceder credenciales al agent/workspace.
