...
- **Próximo batch/corte:** **BLOCKED en deploy Worker**. Wrangler falló antes de mutar con Cloudflare `10000 Authentication error` / `9109 Invalid access token`; health público posterior sigue 200/OK sobre la versión previa. No se intentaron Admin VPS ni prerelease. Próximo: restaurar/autorizar credencial Workers válida y reanudar Worker → health → Admin → prerelease, sin rebuild mientras HEAD siga `9274577`. No incluye login/link ni provider/dictado.
