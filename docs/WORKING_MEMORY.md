...
| `011-selection-transform-and-recovery-ergonomics` | active but paused behind real-selection smoke gate: fixture-first routing/transforms, safe paste-last, explicit host command boundary, and redacted target metadata implemented; real selected-text read still gated | `specs/011-selection-transform-and-recovery-ergonomics/tasks.md` |
| `012-fixvox-dock-dictation-key` | complete through larger parity follow-ups: Skin4-like dock, Rust/Tauri shell, tray/context menu, gated native Alt+Space hook, companion panel, bounded result history, side-by-side smoke, and real `paste_sent` E2E | `specs/012-fixvox-dock-dictation-key/tasks.md` |
...
`012` Checkpoint E y follow-ups principales quedaron cerrados: dock parity principal, tray/context menu, hide-on-close, ruta gated nativa de Alt+Space, companion panel, result history local y frontera host-owned de seleccion. Proximo trabajo probable: separar companion window real tipo Fixvox, completar `011` T039 real selected-text smoke gated, conectar presets a selection-transform real, o harden/soak Alt+Space antes de hacerlo default.
