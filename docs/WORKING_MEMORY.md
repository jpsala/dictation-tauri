...
| `013-verified-paste-observer` | active: provider-free seam complete and native Windows observer implemented behind `VITE_ENABLE_NATIVE_PASTE_OBSERVER=1`; controlled smoke still pending | `specs/013-verified-paste-observer/tasks.md` |
...
28. Computer-use quedo instalado (`npm:@amaster.ai/pi-computer-use`) y probado: `computer_use_get_window_state` puede inspeccionar `Dictation Dock` y Notepad. Smoke asistido 2026-06-23 confirmo via UIA que Notepad contiene el texto insertado; dock compacto muestra `Review ready` pero no una etiqueta separada `paste_observed`.
29. Proximo recomendado: agregar evidencia machine-readable/redacted del delivery status o UI/log visible para confirmar promocion `paste_observed`; alternativa si JP prioriza app usable: tray/background como siguiente spec/checkpoint.
30. Checks recientes: `npm run test:pipeline` OK (52 files / 245 tests), focused observer tests OK, `npm run build` OK, `cd src-tauri && cargo check` OK, context index/audit OK con warnings conocidos. Previos: `npm run visual:check` OK (8 tests). `cd src-tauri && cargo test desktop_control` no fue usado como gate: falla en este entorno con `STATUS_ENTRYPOINT_NOT_FOUND`.
