...
- **Próximo batch/corte:** Dirección corregida: JP quiere Trusted Owner Pi sin restricciones, igual a sesión VPS como `jpsal`; aceptó blast radius y eligió `Sin restricciones`. Implementación local: `PI_CHAT_UNRESTRICTED_OWNER=1`, remote sandbox off, Pi normal/home/config/extensions/tools/creds, `--approve`, OAuth owner reciente en health/prompt/command, single-task/stop/30m idle. VPS RPC offline real PASS; 27/27 tests. Release tools quedan off y brokers provisionados sin uso. Falta commit/push y gate exacto para Admin deploy+env switch+restart; luego smoke read-only y prompt real sin mutación antes de usar libremente.
