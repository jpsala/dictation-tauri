...
- **Próximo batch/corte:** Trusted Owner Pi `ACTIVE` y cierre completo. Base `91c7b3b`; raw prompt fix `d92c09b`; backups Admin `20260718-215058` y `20260718-220037`; env receipt owner-mode. Pi normal UID/HOME/config/extensions/skills/tools/creds/cwd `jpsal`, `--approve`, sin wrappers/guardrails internos; OAuth owner reciente + single-task/stop/30m. Env `guardrails=[]`, warning honesto, health/proceso PASS; prompts UI `OWNER_MODE_OK` y `RAW_OWNER_PROMPT_OK`, 0 tools. Checkout VPS dirty 19/HEAD `0ae95311` preservado. Único gap de paridad: Windows/Chrome local requiere relay aparte.
