...
| `011-selection-transform-and-recovery-ergonomics` | active behind real-selection smoke gate: fixture-first routing/transforms, safe paste-last, explicit host command boundary, redacted target metadata, and best-effort UIA selected-text read implemented; T039 smoke still open | `specs/011-selection-transform-and-recovery-ergonomics/tasks.md` |
...
`012` Checkpoint E y follow-ups principales quedaron cerrados: dock parity principal, tray/context menu, hide-on-close, Alt+Space default en Windows con fallback, companion window first-slice, result history local, UIA selected-text read y observer bounded. Proximo trabajo probable: completar `011` T039 real selected-text smoke, pasar E2E post-default sin foreground-lock, sincronizar companion window real, conectar presets a selection-transform/replace-selection, y probar observer antes de reclamar `paste_observed`.
