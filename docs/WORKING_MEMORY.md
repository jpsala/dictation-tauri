...
| `011-selection-transform-and-recovery-ergonomics` | active but paused behind real-selection smoke gate: fixture-first routing/transforms, safe paste-last, and explicit host command boundary implemented; real selected-text read still gated | `specs/011-selection-transform-and-recovery-ergonomics/tasks.md` |
| `012-fixvox-dock-dictation-key` | complete through dock parity follow-ups: Skin4-like dock, Rust/Tauri shell, tray/context menu, gated Alt+Space config path, side-by-side smoke, and real `paste_sent` E2E | `specs/012-fixvox-dock-dictation-key/tasks.md` |
...
`012` Checkpoint E y follow-ups principales quedaron cerrados: dock parity principal, tray/context menu, hide-on-close, ruta gated de Alt+Space y frontera host-owned de seleccion. Proximo trabajo probable: enriquecer tray/context menu con presets/history/settings, implementar hook robusto Alt+Space si se aprueba, o completar `011` T039 real selected-text smoke gated.
