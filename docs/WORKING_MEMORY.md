...
- **Próximo batch/corte:** Batch 4 grep/find/ls read-only `DONE`: source `67b0643`, run runtime-only `20260718-030745`, mirror sin cambio, rg no-shell y bounded, canonical auth por match, secrets/DB/symlink/glob traversal bloqueados; broker + RPC Linux offline smokes PASS con args explícitos y no builtins. Siguiente requiere elección/gate: git/deploy credentials específicas o relay Chrome. No conceder Docker/sudo/SSH ni homes/stores.
