...
`012` Checkpoint E y Lote 1 follow-up quedaron cerrados para dock parity principal: controles separados verde/azul/rojo, idle rounded hit-region, resize state-aware, preset/assistant visual-only y `paste_sent` honesto. Proximo trabajo probable: context menu/tray/background, Alt+Space gated, o volver a `011` seleccion/recovery gated.
