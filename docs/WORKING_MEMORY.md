...
23. Dev dock debe quedar siempre corriendo durante trabajo de usabilidad: usar `powershell -ExecutionPolicy Bypass -File scripts/dev-dock.ps1`, y `-Restart` tras cambios Rust/Tauri/config/hotkey/window; renderer/CSS/TS hot-refresh via Vite. `-Restart` ahora mata stale `dictation-tauri` + duenos del puerto 1420/Vite antes de relanzar.
