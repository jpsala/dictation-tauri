...
- **Próximo batch/corte:** Batch 5 foundation `8bb6668`; scope elegido: Dictation push + Admin deploy. Helper Admin exact-hash/clean-main/manifest/backup/restart/health/rollback está local con tests verdes, aún sin commit. Feature default/prod `0`; bash bypass bloqueado. Próximo: commit helper y gate separado para crear `fixvox-release`, deploy key write sólo Dictation, sockets/services/config root-owned. Primer commit/push/deploy smoke requiere otro gate. No conceder credenciales al agent/workspace.
