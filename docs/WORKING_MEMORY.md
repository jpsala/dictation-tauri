...
- **Próximo batch/corte:** Batch 3 rollout/sync durable está listo local/provider-free: dry-run default, gate `-ConfirmProduction`, sync separado, manifest/hash/retry, backup/rollback, dirty/secret rejection, atomic mirror swap y postchecks. Falta commit/push y gate exacto antes de ejecutar `-ConfirmProduction -SyncMirrors`. Luego: grep/find/ls, git/deploy credentials específicas y relay Chrome/desktop. No conceder Docker/sudo/SSH ni homes/stores.
