...
| `012-fixvox-dock-dictation-key` | complete through Checkpoint E: close Fixvox Skin4 dock parity, Rust/Tauri Windows-shell fidelity, side-by-side smoke, and post-parity real `paste_sent` E2E; remaining deviations accepted as follow-ups | `specs/012-fixvox-dock-dictation-key/tasks.md` |
...
`012` Checkpoint E quedo cerrado: dock cercano a Fixvox Skin4, shell Rust/Tauri no-activate/toolWindow, smoke side-by-side y E2E real post-parity con `paste_sent`. Proximo trabajo debe elegirse: follow-ups aceptados de dock (enter-submit, hit-region, resize, context/preset/assistant), Alt+Space/tray, o volver a `011` seleccion/recovery gated.
