...
24. `Needs attention` del smoke se diagnostico: faltaba `.env` local y managed Fixvox requeria install/device; se copio `.env` gitignored desde `C:\dev\fixvox` sin imprimir secretos y Rust ahora cae a direct Groq BYOK si managed setup falla pero Groq esta configurado. Computer-use smoke 2026-06-23 paso con target controlado: `Ctrl+Shift+F9` creo `capture-native-1782247153935.wav` e inserto 34 bytes redacted; `Alt+Space` creo `capture-native-1782247201829.wav` e inserto 34 bytes redacted; clipboard sentinel restaurado. Semantica sigue `paste_sent`, no `paste_observed`.
25. Proximo recomendado: native observer Windows gated (UI Automation o fixture controlado) o target heuristics antes de llamar al paste “verificado”; alternativa si JP prioriza app usable: tray/background como siguiente spec/checkpoint.
26. Checks recientes: `npm run test:pipeline` OK (50 files / 241 tests), `npm run build` OK, `npm run visual:check` OK (8 tests), `cd src-tauri && cargo check` OK. Previos: `bun scripts/context-index.ts && bun scripts/agent-context-audit.ts` OK con 4 warnings conocidos. `cd src-tauri && cargo test desktop_control` no fue usado como gate: falla en este entorno con `STATUS_ENTRYPOINT_NOT_FOUND`.
