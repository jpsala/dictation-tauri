...
- **Estado:** `ready`.
- **Plan:** `docs/tracks/fixvox-account-profile-inheritance-hotfix.md`.
- **Próximo batch:** **Batch 1 — Promover runtime y verificar herencia account-wide**.
...
- La otra PC confirmó la deuda de account linkage: OAuth terminaba pero la cuenta canónica no tenía profile y caía a `Basic`, mientras la PC original conservaba `Pro` sólo por device. Una reparación productiva provider-free, respaldada y auditada dejó la cuenta Google canónica en `Pro` account-wide y la PC nueva resuelve `source=account`; el runtime versionado que evita placeholders compartidos y muestra callback visible está pendiente de promoción.
...
| `fixvox-account-profile-inheritance-hotfix.md` | Activa: account-wide `Pro` reparado en producción; falta promover el fix versionado de identidad/callback/Admin. |
