...
- **Estado:** Batch 2 remoto `DONE`. Pi Chat producción usa `fixvox-agent` (OpenAI Codex/sessions/audit, sin workspace) + `fixvox-workspace` (mirrors/tools, sin OAuth) por sockets 0660 y custom tools `--no-builtin-tools`. Direct read cruzado falla; broker roots/secrets fail-closed. Constelaciones read broker sandboxed/SQLite read-only responde turnos redacted. Provider read smoke, tool de turnos, approve-write, cancel-no-write, `agent_settled` y audit sin raw fields pasaron. Backup runtime `20260718-022000`; Admin backup `20260718-021738.tar.gz`.
- **Próximo batch/corte:** paridad VPS queda activa. Pendiente separado: relay Chrome/desktop local con presence/revocación; sync controlado de mirrors y credenciales git/deploy específicas si se quieren push/deploy desde el agente. No conceder Docker/sudo/SSH ni acceso directo a homes/stores.
