...
- **Próximo batch/corte:** Release provisioning `DONE` run `20260718-034947`: `fixvox-release`, Dictation-only write deploy key, root configs/units installed disabled, Admin release feature off, isolation/key auth PASS; backup+key ID guardados. Mirror clean pero stale `5400a16` vs remote `df021fc`, así push queda fail-closed. Falta commit de known_hosts/safe.directory fixes y gate separado para sync mirror + enable services/tools + status/diff smoke read-only. Primer commit/push y deploy Admin son gates posteriores independientes.
