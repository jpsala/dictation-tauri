...
- **Próximo batch/corte:** Batch 4 grep/find/ls brokered read-only listo local: no builtins/filesystem directo; rg sin shell con timeout/output/file/match caps; globs traversal-safe; ls/find canonicalizados; `.env`/stores/sessions/DB excluidos; tests provider-free verdes. Falta commit/push y gate runtime-only `-ConfirmProduction` sin mirror sync. Después: gates separados para git/deploy y relay Chrome. No conceder Docker/sudo/SSH ni homes/stores.
