...
`011` T039 quedo cerrado con smoke real redacted de selected-text UIA por IPC de producto. `012` Checkpoint E y follow-ups principales quedaron cerrados: dock parity principal, tray/context menu, hide-on-close, Alt+Space default en Windows con fallback, companion window sincronizada/actionable first-slice, result history local, UIA selected-text read y observer bounded. Observer interno hardeneado y E2E controlado `paste_observed` paso; proximo trabajo probable: conectar presets a selection-transform/replace-selection (gated para seleccion/replace real) o pasar mas E2E post-default/Alt+Space/companion.
