...
- **Plan:** `docs/tracks/fixvox-product-first-self-hosted-contract-plan.md`.
- **Próximo batch:** **Batch 1 — Mapa Consumidor/Disposición**. Clasificar 73 fixtures/72 rutas únicas + scheduled por consumidor y `canonical`/`redesign`/`temporary-compat`/`drop`.
- **Perfil recomendado:** **Implementador**, motor manual staged y un único owner. Salir de esta sesión y ejecutar `pi-menu -Preset implementer -Session new`; en la sesión nueva usar `/flow` → **Implementar un batch**. Ese prompt lee el foco desde WORKING_MEMORY y ejecuta exactamente un batch.
- **Gate:** Batch 1 es provider-free y docs-only. Cada alias necesita consumidor, reemplazo y retiro; una ruta sin consumidor cae por defecto. Sin Taskflow, runtime/clientes, Checkpoint E, VPS, provider real, producción, deploy, import, commit ni push.
