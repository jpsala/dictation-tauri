...
- 2026-06-30: login admin mejorado: `/login` ofrece Google OAuth (`/auth/google/start` + `/auth/google/callback`) con allowlist server-side y token fallback. Google secrets/allowlist viven fuera del repo en `~/.config/dictation-tauri/admin-web.env`; el token fallback fue rotado otra vez. Validado que `/auth/google/start` redirige a Google con callback `https://fixvox.jpsala.dev/auth/google/callback`. Browser smoke real en Vivaldi llego a Google pero fallo con `Error 400: redirect_uri_mismatch`; falta agregar ese callback en el OAuth client de Google Cloud antes de que el login Google quede operativo end-to-end.
