...
Screenshots reales posteriores mostraron una segunda regresión: a `871px`, una regla desktop `.pi-grid` declarada después del media query volvía a forzar chat + actividad en dos columnas, generaba overflow horizontal, ocultaba el sidebar por scroll y dejaba código sin wrap. `e7c59eb` agrega el override responsive al final del cascade, fuerza `min-width: 0`, apila Actividad debajo del chat y envuelve bloques de código sólo en viewport angosto. Screenshot headless `871×625` pasó con `scrollWidth === clientWidth`, drawer visible y actividad apilada.

El primer prompt real reveló una tercera regresión: Pi RPC emite `message_end` también para el mensaje `user`; `extractAssistantText()` no validaba `role`, por lo que el bubble del agente mostraba literalmente el prompt interno + contexto/guardrails mientras Pi seguía trabajando. El proceso quedó explorando durante más de cinco minutos y fue detenido por PID sin reiniciar Admin. `b96363e` ignora mensajes no-assistant, sólo acepta `message_end/turn_end` con role assistant, evita que `agent_end` sobreescriba el texto y mantiene SSE abierto hasta `agent_settled`, como define `docs/rpc.md` de Pi 0.80.6. Mock regression y profile smoke cubren user-only message sin eco. Tests Admin 15/15 y smoke PASS. Deploy Admin PASS con backup `/home/jpsal/.local/state/fixvox-admin-backups/20260718-011852.tar.gz`; servicio activo, sin child Pi residual, healthz OK y Browser confirmó el JS productivo. La sesión Browser seguía en login y no se inició OAuth.
