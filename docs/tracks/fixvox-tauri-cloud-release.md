...
- Smoke real T021 2026-06-30: login/session redacted llego a `signed_in`, pero `/desktop/login/link-device` devolvio `FIXVOX_LOGIN_DEVICE_LINK_REJECTED` / `Not found`; source reconnaissance encontro `/desktop/login` y `/desktop/login/status` en `C:/dev/fixvox/proxy/src/index.ts`, pero no handler `link-device`. El cliente fallo cerrado sin `authPolicy` ni capabilities managed. Parche local preparado en `C:/dev/fixvox` (no desplegado): agrega `POST /desktop/login/link-device`, linkea accountId host-side y devuelve auth policy redacted.
...
- Estado: browser-google-oauth-done / tauri-polling-ux-done / cloud-route-implemented-local-not-deployed
...
- Pasos:
  ...
  6. Estado 2026-06-30: Cloud production `/desktop/login` muestra Google OAuth y browser flow completo; Settings no usa modal, abre browser directo, hace polling/focus polling de `/desktop/login/status` via comando host-owned `poll_fixvox_cloud_login`. El host ahora intenta completar link/refresco post-OAuth contra `/desktop/login/link-device` usando state + install/device metadata redacted-safe, persiste el policy snapshot devuelto y Settings deja de mostrar `device link pending` cuando `authPolicy` signed-in llega desde Cloud. Smoke real T021 con aprobacion JP fallo en Cloud: `/desktop/login/link-device` respondio Not found (`FIXVOX_LOGIN_DEVICE_LINK_REJECTED`), por lo que el cliente quedo fail-closed sin authPolicy. Parche local en `C:/dev/fixvox` ya implementa la ruta y tests pasan; falta aprobacion/deploy Cloud para reintentar T021.
- Checks:
...
- 2026-06-30 post-OAuth host link contract: `poll_fixvox_cloud_login` persiste status signed-in redacted y, si no hay `authPolicy` signed-in local, llama host-owned `/desktop/login/link-device` con `state`, `installId` y device metadata para obtener register/policy response, persistir `fixvox-device-state.json` y refrescar Settings. Settings distingue `device link pending` vs `Fixvox policy active`, redakta `userRedacted` aun si un fixture trae ID crudo y `Link signed-in device` reintenta el polling/link en vez de usar register anonimo. Checks: `npm run test:pipeline`, `npm run test:pipeline -- tests/settings tests/voice-dock tests/desktop-control`, `npm run build`, `cd src-tauri && cargo fmt --check && CARGO_TARGET_DIR=target/pi-auth-link cargo check`. Smoke real T021 con aprobacion JP: `artifacts/desktop-control/fixvox-login-link-smoke/20260630-T021-login-link-cdp/report.json` fallo redacted porque Cloud devolvio `FIXVOX_LOGIN_DEVICE_LINK_REJECTED` / `Not found`; `authPolicy` no se persistio y managed capabilities quedaron bloqueadas. Reconocimiento source encontro `/desktop/login` y `/desktop/login/status`, pero no `link-device` en `C:/dev/fixvox/proxy/src/index.ts`. Parche local `C:/dev/fixvox` agrega la ruta y tests `cd C:/dev/fixvox/proxy && bun test` pasan (65/65), pero no hay deploy cloud ni T022.
