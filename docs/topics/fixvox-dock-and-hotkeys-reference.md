...
- Delivery real actual: se guarda el target foreground antes de grabar y al insertar se intenta primero `SendInput` Unicode directo, sin tocar portapapeles. El fallback de clipboard + `Ctrl+V` queda solo opt-in via `DICTATION_TAURI_ALLOW_CLIPBOARD_PASTE_FALLBACK=true` y restaura texto previo best-effort; `Copy transcript` sigue siendo una accion manual explicita.
