...
- Preferencia opt-in `Pegar sin cambiar de ventana`: captura únicamente el input editable que siga en foreground al entregar, no usa el cache como fallback y envía `Ctrl+V` con `focusTargetBeforePaste: false`. Si el foreground cambia o una operación con afinidad al target guardado ya no coincide, falla cerrado sin activar otra ventana ni enviar teclas. El default sigue enfocando el target asegurado para preservar compatibilidad.
