...
- Lote dock move/feedback agrego drag-to-move desde la orb con threshold, persistencia de posicion en app data (`dock-position.v1.json`) y restore/clamp al arrancar. Tambien alinea el feedback post-insert con Fixvox: si delivery llega a `paste_sent` o observer verificado, el dock vuelve a idle en lugar de quedarse mostrando `Transcript ready`/recovery sobre los dots.
